...
function test.DefaultInstall {
  ...
  oc process -f $template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p HAWKULAR_METRICS_HOSTNAME=hawkular-metrics.example.com -p USE_PERSISTENT_STORAGE=false | oc create -f - &> /dev/null
  checkDeployer
  checkTerminated 
...
function test.Redeploy {
  ...
  oc process -f $template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p HAWKULAR_METRICS_HOSTNAME=hawkular-metrics.example.com -p USE_PERSISTENT_STORAGE=false -p REDEPLOY=true | oc create -f - &> /dev/null
  checkDeployer
  checkTerminated
  checkDeployment "Cassandra" 1
  checkDeployment "Hawkular-Metrics" 1
  ...
  oc process -f $template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p HAWKULAR_METRICS_HOSTNAME=hawkular-metrics.example.com -p USE_PERSISTENT_STORAGE=false -p MODE=redeploy | oc create -f - &> /dev/null
  checkDeployer
  checkTerminated
...
function test.Refresh {
  ...
  oc process -f $template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p HAWKULAR_METRICS_HOSTNAME=hawkular-metrics.example.com -p USE_PERSISTENT_STORAGE=false -p MODE=refresh | oc create -f - &> /dev/null
  checkDeployer
  checkTerminated
...
function test.Remove {
  ...
  oc process -f $template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p HAWKULAR_METRICS_HOSTNAME=hawkular-metrics.example.com -p USE_PERSISTENT_STORAGE=false -p MODE=remove | oc create -f - &> /dev/null
...
function test.CassandraScale { 
  ...
  oc secrets new metrics-deployer nothing=/dev/null &> /dev/null
  oc process -f $template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p HAWKULAR_METRICS_HOSTNAME=hawkular-metrics.example.com -p USE_PERSISTENT_STORAGE=false -p CASSANDRA_NODES=2 | oc create -f - &> /dev/null
  ...
  oc process hawkular-cassandra-node-emptydir -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p NODE=3 | oc create -f - 
...
function test.HawkularMetricsFailedStart {
  ...
  oc process hawkular-metrics -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} | oc create -f - &> /dev/null
...
function test.HawkularMetricsInvalidCertificateSkipPreflight {
  ...
  oc process -f $template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p HAWKULAR_METRICS_HOSTNAME=mymetrics.hawkular.org -p USE_PERSISTENT_STORAGE=false -p IGNORE_PREFLIGHT=true | oc create -f - &> /dev/null
