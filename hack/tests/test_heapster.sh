...
function test.DefaultInstall {
  ...
  oc process -f $heapster_template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} | oc create -f - &> /dev/null
  checkDeployer
  checkTerminated 
...
function test.Redeploy {
  ...
  oc process -f $heapster_template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p REDEPLOY=true | oc create -f - &> /dev/null
  checkDeployer
  checkTerminated
  checkDeployment "Heapster" 1
  checkHeapsterAccess
...
function test.RedeployMode {
  ...
  redeployTime=$(date +%s)
  Info "About to redeploy the components"
  oc process -f $heapster_template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p MODE=redeploy | oc create -f - &> /dev/null
  checkDeployer
  checkTerminated
  checkDeployment "Heapster" 1
  checkHeapsterAccess
...
function test.DeployMode {
  undeployAll
  oc secrets new metrics-deployer nothing=/dev/null &> /dev/null
  oc process -f $heapster_template -p IMAGE_PREFIX=${image_prefix} -p IMAGE_VERSION=${image_version} -p MODE=deploy | oc create -f - &> /dev/null
